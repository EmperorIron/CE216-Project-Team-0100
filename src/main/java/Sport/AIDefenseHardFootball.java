...
public class AIDefenseHardFootball extends AIFootball {
    ...
    public ITactic generateStartingTactic() {
        ITactic bestTactic = findBestTactic(TacticFootball.DEFENSIVE_FORMATIONS, getAvailablePlayers());
    ...
    private ITactic findBestTactic(List<String> formations, List<IPlayer> players) {
        ...
        double bestScore = Double.POSITIVE_INFINITY; // We want to minimize xGA
        ...
        if (bestTactic == null) { // Fallback
            bestTactic = new TacticFootball("1-5-3-2");
