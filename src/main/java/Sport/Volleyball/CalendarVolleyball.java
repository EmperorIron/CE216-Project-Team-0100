...
public class CalendarVolleyball extends Calendar {
    ...
    public void displayLeagueTable() {
        ...
        this.teams.sort(Comparator.comparingInt(ITeam::getPoints).reversed()
                .thenComparingInt(ITeam::getGoalDifference).reversed()
                .thenComparingInt(ITeam::getGoalsScored).reversed());

        System.out.println("\n--- Voleybol League Table ---");
        System.out.printf("%-4s %-20s %-3s %-3s %-3s %-3s %-4s %-3s%n", "Pos", "Team", "P", "W", "L", "SF", "SA", "Pts");
        System.out.println("---------------------------------------------------------------");

        int pos = 1;
        for (ITeam team : this.teams) {
            System.out.printf("%-4d %-20s %-3d %-3d %-3d %-3d %-4d %-3d%n",
                    pos++, 
                    team.getName(), 
    ...
    public void advanceToNextWeek() {
        ...
        Training genericTraining = new TrainingVolleyball();
